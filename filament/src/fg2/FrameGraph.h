...
public:
    ...
    public:
        ...
        /**
         * Declare a RenderPass for this pass. All subresource handles get new versions after
         * this call. The new values are available in the returned RenderPass structure.
         * declareRenderPass() implies a write(). If a read() is also needed, it must be
         * issued separately before calling declareRenderPass().
         *
         * @param name  A pointer to a null terminated string.
         *              The pointer lifetime must extend beyond execute()
         * @param desc  Descriptor for the RenderPass.
         * @return      A RenderPass structure containing the new subresource handles as well as
         *              an index to retrieve the concrete RenderPass in the execute phase.
         */
        [[nodiscard]] RenderPass declareRenderPass(const char* name,
                RenderPass::Descriptor const& desc);

        /**
         * Helper to easily declare a RenderPass with a single color attachment.
         ...
         * @param index index of the RenderPass
         * @return handle to the new version of the color attachment subresource
         */
        [[nodiscard]] FrameGraphId<Texture> declareRenderPass(
                FrameGraphId<Texture> color, uint32_t* index = nullptr);
     ...
     * Texture can be used with declareRenderPass(), the resulting concrete RenderPass
     * will be the one passed as argument here, instead of being dynamically created.
     *
     * @param name      A name for the RenderPass
     * @param desc      Descriptor for the imported RenderPass
     * @param target    handle to the concrete RenderPass to import
     * @return          A handle to a Texture
     */
    FrameGraphId<Texture> import(const char* name, RenderPass::Descriptor const& desc,
     ...
     */
    bool isCulled(PassBase const& pass) const noexcept;
...
private:
    ...
    struct ResourceSlot {
        int16_t rid = 0;    // VirtualResource* index in mResources
        int16_t nid = 0;    // ResourceNode* index in mResourceNodes
        int16_t sid =-1;    // ResourceNode* index in mResourceNodes for reading subresource's parent
    };
    void reset() noexcept;
    void addPresentPass(std::function<void(Builder&)> setup) noexcept;
    Builder addPassInternal(const char* name, PassBase* base) noexcept;
...
FrameGraphId<RESOURCE> FrameGraph::createSubresource(FrameGraphId<RESOURCE> parent,
    ...
    auto newParentHandle = static_cast<FrameGraphId<RESOURCE>>(createNewVersionForSubresourceIfNeeded(parent));

    auto* parentResource = static_cast<Resource<RESOURCE>*>(getResource(newParentHandle));
    UniquePtr<VirtualResource> vresource(mArena.make<Resource<RESOURCE>>(parentResource, name, desc), mArena);
    return FrameGraphId<RESOURCE>(addSubResourceInternal(newParentHandle, std::move(vresource)));
}
...
/*
 * Prevent implicit instantiation of methods involving Texture which is a konwn type
 * these are explicitly instantiated in the .cpp file.
 */

extern template void FrameGraph::present(FrameGraphId<Texture> input);

extern template FrameGraphId<Texture> FrameGraph::create(char const* name,
        Texture::Descriptor const& desc) noexcept;

extern template FrameGraphId<Texture> FrameGraph::createSubresource(FrameGraphId<Texture> parent,
        char const* name, Texture::SubResourceDescriptor const& desc) noexcept;

extern template FrameGraphId<Texture> FrameGraph::import(char const* name,
        Texture::Descriptor const& desc, Texture::Usage usage, Texture const& resource) noexcept;

extern template FrameGraphId<Texture> FrameGraph::read(PassNode* passNode,
        FrameGraphId<Texture> input, Texture::Usage usage);

extern template FrameGraphId<Texture> FrameGraph::write(PassNode* passNode,
        FrameGraphId<Texture> input, Texture::Usage usage);
